...
{   
    ...
    int retMenuVal = renderingEngine->drawControlScreen();     //retMenuVal returns 1 if it is finished (This means change screen!) [5 is to not show selected on main menu]
...
bool ControlScreenState::handleKeyboardMouseEvents(SDL_Event &KeyboardMouseEvents)
{
...
void ControlScreenState::handleXboxEvents(int player,XboxController* state)
{   
    if(!lockControls)
    {
...
void ControlScreenState::keySelectLeft()
{
}

void ControlScreenState::keySelectRight()
{
}

void ControlScreenState::keySelectTarget()
{
    //changeState(PLAY);
...
void ControlScreenState::backPressed()
{
    renderingEngine->startTransition(RenderingEngine::FADE_OUT);
}

ControlScreenState* ControlScreenState::getInstance()
{    
    printf("Controller state\n");
    static ControlScreenState controlScreenState;
    controlScreenState.changeState(CONTROLS);
